...
public class UserController {
    ...
    public User selectOne(@PathVariable Integer user_id) {
        return userMapper.getById(user_id);
    }

    // 后端接口，处理密码修改请求
    @PostMapping("/psw/{user_id}")
    public Integer savePsw(@PathVariable Integer user_id, @RequestBody Map<String, String> passwordData) {
        String oldPassword = passwordData.get("user_psw");
        String newPassword = passwordData.get("user_newPsw");

        return userMapper.updatePSW(user_id, newPassword, oldPassword);
    }
