        ...
        <div style="height: 2000px;">
          ...
          left: 1190px;" @click="showDialog"
            ...
            <el-dialog
                ...
                @close="handleClose">
              <span>展品质量与丰富度</span>
              <el-rate v-model="exhibits" :texts="texts" show-text></el-rate>
              <br>
              <span>舒适度和整洁度</span>
              <el-rate v-model="comfort" :texts="texts" show-text></el-rate>
              <br>
              <span>游览体验和服务质量</span>
              <el-rate v-model="service" :texts="texts" show-text></el-rate>
              <br>
              <span>设施便利性</span>
              <el-rate v-model="facility" :texts="texts" show-text></el-rate>
              <br>
              <span>活动丰富度</span>
              <el-rate v-model="activity" :texts="texts" show-text></el-rate>
...
export default {
  ...
  methods: {
    getRated() {
      fetch('http://localhost:9091/rating/get?m_id=' + this.m_id + '&user_id=' + this.user.user_id).then(res => {
        if (res.status === 500) {
    ...
    getFavorites() {
      fetch('http://localhost:9091/favorites/get?m_id=' + this.m_id + '&user_id=' + this.user.user_id).then(res => {
        if (res.status === 500) {
          this.$notify.error('系统错误')
          return {}
        }
    ...
    toggleFavorite() {
      if (!this.isFavorite) {
        fetch('http://localhost:9091/favorites/add?m_id=' + this.m_id + '&user_id=' + this.user.user_id).then(res => {
          if (res.status === 500) {
            this.$notify.error('系统错误')
            return {}
          }
        ...
        fetch('http://localhost:9091/favorites/delete?m_id=' + this.m_id + '&user_id=' + this.user.user_id).then(res => {
          if (res.status === 500) {
            this.$notify.error('系统错误')
            return {}
          }
    ...
    confirmDialog() {
      ...
      let data = {
        ...
        m_id: this.m_id,
        r_exhibits: this.exhibits,
        r_comfort: this.comfort,
        r_service: this.service,
        r_facility: this.facility,
        r_activity: this.activity
      }
      //验证通过
      this.$request.post('/rating/add', data).then(res => {
    ...
    fetchMuseumData() {
      ...
      return fetch("http://localhost:9091/museum/find?" +
          "&m_id=" + pageId + "&user_id=" + this.user.user_id
